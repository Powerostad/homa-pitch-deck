...
import {
  ArrowRight, Smartphone, Armchair, Move3d,
        ...
        <div className="w-[360px] flex flex-col border-r border-gray-100 relative z-20 bg-white shadow-xl transition-all duration-500">
           ...
           <div className="flex-1 overflow-y-auto custom-scrollbar p-6 flex flex-col">
              ...
              <div className="w-full aspect-square relative mb-6 rounded-2xl overflow-hidden border border-gray-100 shadow-sm">
                  <img 
                    src={demoStep >= 2 ? chairImg : moodboardThumb} 
                    alt="Product" 
           ...
           <div className={`absolute bottom-8 left-1/2 -translate-x-1/2 transition-opacity duration-300 z-30 ${demoStep >= 6 ? 'opacity-0' : 'opacity-100'}`}>
              <div className="bg-black/80 backdrop-blur text-white px-6 py-3 rounded-full flex items-center gap-3 animate-pulse">
                 ...
                 <span className="font-bold text-xs uppercase tracking-widest">
                    {demoStep === 0 ? 'Next > Mood' : demoStep === 1 ? 'Next > Item' : demoStep === 2 ? 'Next > Render' : demoStep === 3 ? 'Next > Step 4' : demoStep === 4 ? 'Next > Step 6' : 'Next > Final'}
                 </span>
              </div>
           </div>
...
// --- SLIDE 9: GTM (AUTOMATIC BEFORE/AFTER SLIDER) ---
export const GTMSlide: React.FC<SlideProps> = ({ isActive }) => {
  const [sliderPosition, setSliderPosition] = useState(50);
  const requestRef = useRef<number>(0);
  const startTimeRef = useRef<number>(0);

  // Auto-animate slider with a sine wave
  useEffect(() => {
    if (!isActive) return;
    ...
    const animate = (time: number) => {
      ...
      // Speed factor: 0.001 (slower is smaller)
      // Math.sin creates -1 to 1. 
      // Normalize to 0 to 100.
      const rawPos = Math.sin(progress * 0.001); 
      const newPos = (rawPos + 1) * 50; // Map -1..1 to 0..100
  ...
  return (
    <div className="h-full flex flex-col justify-center items-center bg-black">
      <h2 className="text-4xl font-bold mb-8 text-white">استراتژی ورود به بازار (Phase 1: Try-On)</h2>
      
      <div className="relative w-[1000px] h-[600px] rounded-xl overflow-hidden shadow-2xl border border-zinc-800">
        
        {/* Layer 1: After (Furnished) - The "Reveal" */}
        <div 
           className="absolute inset-0 bg-cover bg-center"
           style={{ backgroundImage: `url('https://images.unsplash.com/photo-1556228453-efd6c1ff04f6?q=80&w=2070&auto=format&fit=crop')` }}
        >
           <div className="absolute inset-0 bg-black/40 flex items-center justify-center">
              <span className="text-white text-6xl font-black drop-shadow-xl tracking-tighter">AFTER</span>
           </div>
        </div>

        {/* Layer 2: Before (Empty) - Masked */}
        <div 
          className="absolute inset-0 bg-cover bg-center border-r-4 border-white"
          style={{ 
            backgroundImage: `url('https://images.unsplash.com/photo-1600607686527-6fb886090705?auto=format&fit=crop&w=2000&q=80')`,
            width: `${sliderPosition}%` 
          }}
        >
           <div className="absolute inset-0 bg-black/20 flex items-center justify-center">
              <span className="text-white text-6xl font-black drop-shadow-xl tracking-tighter">BEFORE</span>
           </div>
        </div>

        {/* Slider Handle */}
        <div 
          className="absolute top-0 bottom-0 w-12 flex items-center justify-center -translate-x-1/2 shadow-[0_0_30px_rgba(0,0,0,0.5)] z-20"
